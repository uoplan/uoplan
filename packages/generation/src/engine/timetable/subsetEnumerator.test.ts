...
import { createSeededRng } from "@uoplan/domain/seededRandom";
...
import type { Catalogue, SchedulesData } from "@uoplan/domain/dataTypes";
...
function buildCache(): ReturnType<typeof buildDataCache> {
  const catalogue: Catalogue = {
    courses: [
      { code: normalizeCourseCode("PIN 1000"), title: "Pinned", credits: 3, description: "" },
      { code: normalizeCourseCode("OPT 1000"), title: "Opt A", credits: 3, description: "" },
      { code: normalizeCourseCode("OPT 2000"), title: "Opt B", credits: 3, description: "" },
      { code: normalizeCourseCode("OPT 3000"), title: "Opt C", credits: 3, description: "" },
    ],
    programs: [],
  };
  const schedules: SchedulesData = {
    termId: "0",
    schedules: [
      makeSchedule(normalizeCourseCode("PIN 1000"), {
        LEC: [makeSection("LEC", "A", [{ day: "Mo", start: 600, end: 690 }])],
      }),
      makeSchedule(normalizeCourseCode("OPT 1000"), {
        LEC: [makeSection("LEC", "A", [{ day: "Tu", start: 600, end: 690 }])],
      }),
      makeSchedule(normalizeCourseCode("OPT 2000"), {
        LEC: [makeSection("LEC", "A", [{ day: "We", start: 600, end: 690 }])],
      }),
      makeSchedule(normalizeCourseCode("OPT 3000"), {
...
function input(seed: number, optional: string[]) {
  return {
    pinned: [normalizeCourseCode("PIN 1000")],
    optional,
    targetCount: 3,
    cache,
    pipeline,
...
describe("enumerateSubsetTimetables", () => {
  it("always includes every pinned course and picks exactly targetCount courses", () => {
    const schedule = firstSubsetArrangement(
      input(1, [
        normalizeCourseCode("OPT 1000"),
        normalizeCourseCode("OPT 2000"),
        normalizeCourseCode("OPT 3000"),
      ]),
    );
    expect(schedule).not.toBeNull();
    const codes = schedule!.enrollments.map((e) => e.courseCode).sort();
    expect(codes).toHaveLength(3);
    expect(codes).toContain(normalizeCourseCode("PIN 1000"));
  });
  ...
  it("preserves the given optional order (no internal re-sort) so subset is deterministic for an order", () => {
    ...
    const a = firstSubsetArrangement(
      input(1, [
        normalizeCourseCode("OPT 1000"),
        normalizeCourseCode("OPT 2000"),
        normalizeCourseCode("OPT 3000"),
    ...
    expect(setA).toEqual([
      normalizeCourseCode("OPT 1000"),
      normalizeCourseCode("OPT 2000"),
      normalizeCourseCode("PIN 1000"),
    ]);

    const b = firstSubsetArrangement(
      input(1, [
        normalizeCourseCode("OPT 3000"),
        normalizeCourseCode("OPT 2000"),
        normalizeCourseCode("OPT 1000"),
  ...
  it("yields nothing when a pinned course cannot be scheduled", () => {
    const bad = {
      ...input(1, [normalizeCourseCode("OPT 1000"), normalizeCourseCode("OPT 2000")]),
      pinned: [normalizeCourseCode("DOES 9999")],
    };
