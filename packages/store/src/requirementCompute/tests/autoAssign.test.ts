...
import type { AutoAssignReqMeta, DataCache, RemainingRequirement } from "@uoplan/core";
import {
...
describe("autoAssign", () => {
  ...
  describe("compareReqPreference", () => {
    it("prefers stricter subset of candidates", () => {
      const a: AutoAssignReqMeta = {
        reqId: "1",
        type: "course",
        creditsNeeded: 3,
        candidatesNorm: new Set(["A", "B"]),
      };
      const b: AutoAssignReqMeta = {
        reqId: "2",
        type: "course",
        creditsNeeded: 3,
        candidatesNorm: new Set(["A", "B", "C"]),
      };
    ...
    it("prefers higher priority type if not strict subset", () => {
      const a: AutoAssignReqMeta = {
        ...
        type: "course",
        creditsNeeded: 3,
        candidatesNorm: new Set(["A"]),
      };
      const b: AutoAssignReqMeta = {
        reqId: "2",
        type: "elective",
        creditsNeeded: 3,
        candidatesNorm: new Set(["B"]),
      };
  ...
  describe("getAutoSelectedSingleEligibleCompleted", () => {
    it("auto-assigns completed courses to matching requirements", () => {
      ...
      const result = getAutoSelectedSingleEligibleCompleted(
        remaining,
        completed,
        mockCache,
        {},
