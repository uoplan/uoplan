...
import { extractPrereqSentence, parseCoursePrerequisites } from "./catalogue/prerequisites.ts";
...
describe("extractPrereqSentence", () => {
  ...
  it("handles B.Com abbreviation without truncating", () => {
    const raw =
      "Prerequisite: Course reserved for students in a B.Com program. Other students may not enroll.";
  ...
  it("handles French Baccalauréat abbreviation", () => {
    ...
    expect(extractPrereqSentence(raw)).toBe(
      "Réservé aux étudiants inscrits au Baccalauréat en musique (B.Mus.)",
    );
  });
