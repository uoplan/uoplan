...
fetch_latest_version() {
  ...
  if command -v curl > /dev/null 2>&1; then
    version=$(curl -fsSL "$url" | grep '"tag_name"' | grep 'cli-rs/' | head -1 | sed 's/.*"cli-rs\/\(v[^"]*\)".*/\1/')
  elif command -v wget > /dev/null 2>&1; then
    version=$(wget -qO- "$url" | grep '"tag_name"' | grep 'cli-rs/' | head -1 | sed 's/.*"cli-rs\/\(v[^"]*\)".*/\1/')
  else
...
main() {
  ...
  URL="https://github.com/${REPO}/releases/download/cli-rs%2F${VERSION}/${ARCHIVE}"
