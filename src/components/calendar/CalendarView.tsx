...
import type { ProfessorRatingsMap } from '../../lib/professorRatings';
import {
...
export function CalendarView({
  ...
  // Use the swap modal hook for all swap-related state and logic
  const swap = useSwapModal(getSwapCandidates, selectedIndex, cache);
  ...
  // Use the calendar events hook to transform schedule to events
  const events = useCalendarEvents(currentSchedule, professorRatings);

  // Reference week for calendar display
  const referenceWeekStart = useMemo(
    () => startOfWeek(new Date(), { weekStartsOn: 0 }),
    []
  );

  // Build the swap dropdown UI
  const swapDropdown = (() => {
    ...
    return (
      <Select
        ...
        onChange={(v: string | null) => {
          ...
          onSwap(selectedIndex, swap.modalState.enrollmentIndex, v);
          swap.closeModal();
        }}
        renderOption={({ option }) => {
